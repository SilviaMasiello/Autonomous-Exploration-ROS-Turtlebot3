...
def save_positions_to_excel():
    ...
    with save_lock:
        ...
        if valid_positions:  # Salva solo se ci sono dati validi
            ...
            df.to_excel('/root/ws/progetto/catkin_ws/src/pkg_project/positions.xlsx', index=False)
            rospy.loginfo("Posizioni salvate in positions.xlsx")
...
def main():
    ...
    try:
        while not rospy.is_shutdown():
            ...
            # Stampa le statistiche di esplorazione
            print_exploration_stats()
 
            if has_obstacle_front():  # If there are obstacles in front
                avoid_obstacle()  # Avoid obstacles
            elif has_free_space():  # If there is free space in front
            ...
            # Salva periodicamente ogni 60 secondi
